...
class mod_mdgen_mod_form extends moodleform_mod
{
    function definition()
    {
        ...
        $mform->addElement('editor', 'mdgen', "Seiteninhalt", null, mdgen_get_editor_options($this->context));
        $mform->addRule('mdgen', get_string('required'), 'required', null, 'client');

        //-------------------------------------------------------

        $mform->addElement('header', 'information', "Informationen zur Syntax");
        $mform->addElement(
            'static',
            'syntaxinfos',
            get_string('headlines', 'mdgen'),
            get_string('headlinesdes', 'mdgen')
        );

        $mform->addElement(
            'static',
            'syntaxinfos',
            get_string('bolditalic', 'mdgen'),
            get_string('bolditalicdes', 'mdgen')
        );

        $mform->addElement(
            'static',
            'syntaxinfos',
            get_string('lists', 'mdgen'),
            get_string('listsdes', 'mdgen')
        );

        $mform->addElement(
            'static',
            'syntaxinfos',
            get_string('code', 'mdgen'),
            get_string('codedes', 'mdgen')
        );

        $mform->addElement(
            'static',
            'syntaxinfos',
            get_string('newslide', 'mdgen'),
            get_string('newslidedes', 'mdgen')
        );

        $mform->addElement(
            'static',
            'syntaxinfos',
            get_string('link', 'mdgen'),
            get_string('linkdes', 'mdgen')
        );

        $mform->addElement(
            'static',
            'syntaxinfos',
            get_string('slide', 'mdgen'),
            get_string('slidedes', 'mdgen')
        );

        $mform->addElement(
            'static',
            'syntaxinfos',
            get_string('more', 'mdgen'),
            get_string('moredes', 'mdgen')
        );
        ...
        if (isset($this->current->legacyfiles) and $this->current->legacyfiles != RESOURCELIB_LEGACYFILES_NO) {
            $options = array(
                RESOURCELIB_LEGACYFILES_DONE   => get_string('legacyfilesdone', 'mdgen'),
    ...
    public function data_preprocessing(&$defaultvalues)
    {
        if ($this->current->instance) {
            ...
            $defaultvalues['mdgen']['text']   = file_prepare_draft_area(
                $draftitemid,
                $this->context->id,
                'mod_mdgen',
                'content',
                0,
                mdgen_get_editor_options($this->context),
